...
if ~exist('FAVARopt.CImethod','var')
    FAVARopt.CImethod = 0;
end
...
GARCHresiduals = FAVAR.GARCHresiduals;
...
%% STEP 1: choose the method and generate the residuals
    %if GARCHresiduals == 1  % Garch residual based bootstrapping available
    % but very computionally heavy.
...
MED(:,:,:)  = median(FEVD(:,:,:,:),4);
MEDx(:,:,:) = median(FAVD(:,:,:,:),4);
INF(:,:,:)  = max(MED(:,:,:) + std(FEVD,0,4) * (-nStd), 0);
SUP(:,:,:)  = MED(:,:,:) + std(FEVD,0,4) * nStd;
INFx(:,:,:) = MEDx(:,:,:) + std(FAVD,0,4) * (-nStd);
SUPx(:,:,:) = min(MEDx(:,:,:) + std(FAVD,0,4) * nStd, 1);
else
pctg_inf = (100-pctg)/2; 
...
INFx(:,:,:) = prctile(FAVD(:,:,:,:),pctg_inf,4);
SUPx(:,:,:) = prctile(FAVD(:,:,:,:),pctg_sup,4);
MEDx(:,:,:) = prctile(FAVD(:,:,:,:),50,4);
end
