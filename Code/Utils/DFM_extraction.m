function [Factor,eigenvector] = DFM_extraction(X, plag, K, type)
...
% plag: Number of lags in the DFM; order of Lambda(L)
...
A(1:K,1:K*(plag-1)) = L';
e = F_endo - F_lag*A(1:K,1:K*(plag-1))';  % Factor error                                % Error Comp.
H = diag(diag(cov(e)));
Q(1:K,1:K) = H;                           % Comp.
idiComp = X-CC;                           % Idiosynchratic Comp.
R = diag(diag(cov(idiComp)));         % R diagonal
